...
class FirstFollowSet {
    ...
    _findFirst(raw_grammar) {
        ...
        this._findFollow(raw_grammar, this.first);
    ...
    _findFollow(userinput, first) {
        ...
        for (var fnum = 0; fnum < keys.length; fnum++) {//初始化  把follow集合的key写上去
            this.follow_set[keys[fnum]] = [];
        }

        this.follow_set[userinput[0].head].push("$");
                ...
                RHS.forEach((currentbody)=> {
                    for (let i = 0; i < currentbody.length; i++) {
                        ...
                        if (this._isNonterminal(B)) {//如果是非终结符的话
                            ...
                            var size = this.follow_set[B].length;
                            var Barr = this.follow_set[B];
                            ...
                            var newsize = this.follow_set[B].length;
                            if (size != newsize) {
                                change = true;
                            }
        ...
        console.log("follow" + this.follow_set);
